...
RepNmotif::RepNmotif(string i_PathFileInput,	int maxLevelNmotifs) {
    ...
    readFileandParseSequenceStructure(i_PathFileInput, headers, structures, sequences,isCircular);
    if (headers.size()<2)
...
void RepNmotif::readFileandParseSequenceStructure(string filePath,
		...
		vector<string>& sequences, bool& isCircular) {
	...
	while (getline(myfile, line)) {
		if (line[0] == '>') {
			if (tempLine != "") {
				parseTempLine(tempLine, tempLineHeader, tempSequence,
						tempStructure);
	...
	parseTempLine(tempLine, tempLineHeader, tempSequence, tempStructure);
    parseSequenceStructure(tempStructure,tempSequence);
	structures.push_back(tempStructure);
	sequences.push_back(tempSequence);
	myfile.close();
...
/******************************************************************************/
void parseTempLine(string tempLine, string tempLineHeader, string& tempSequence,
		string& tempStructure) {
	int sizeStructAndSeq, i, tempLinesizeWithSpecChar,
			diff_tempLinesizeWithSpecChar_tempLinesize, step;
	i = tempLinesizeWithSpecChar = 0;

	//remove all type of space
	tempLine.erase(
			std::remove_if(tempLine.begin(), tempLine.end(),
					(int (*)(int))isspace), tempLine.end());
	string ASCIIwithoutSpaceCar =
			"!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";

	if (tempLine.find_first_not_of(ASCIIwithoutSpaceCar) != string::npos) {
		while (tempLine.c_str()[i]) {
			if ((tempLine.c_str()[i] & 0xC0) != 0x80) {
				tempLinesizeWithSpecChar++;
			}
			i++;
		}
		diff_tempLinesizeWithSpecChar_tempLinesize = tempLine.length()
				- tempLinesizeWithSpecChar;
		step = (tempLine.length() / 2)
				+ diff_tempLinesizeWithSpecChar_tempLinesize;
		tempSequence = tempLine.substr(0, step);
		tempStructure = tempLine.substr(step, tempLinesizeWithSpecChar / 2);
	} else {
		sizeStructAndSeq = tempLine.length() / 2;
		tempStructure = tempLine.substr(sizeStructAndSeq, sizeStructAndSeq);
		tempSequence = tempLine.substr(0, sizeStructAndSeq);

		if (tempStructure.length() != tempSequence.length()) {
			cerr
					<< "Reading structures and sequences issues : sequence and structure length are unequal. Please check the structure: "
					<< tempLineHeader << ". ";
			exit(EXIT_FAILURE);
		}
